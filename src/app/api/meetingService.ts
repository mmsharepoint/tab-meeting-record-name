...
export const meetingService = (options: any): express.Router => {
    ...
    const bearerStrategy = new BearerStrategy({
        ...
        clientID: process.env.PRONOUNCENAME_APP_ID as string,
        audience: `api://${process.env.HOSTNAME}/${process.env.PRONOUNCENAME_APP_ID}` as string,
        loggingLevel: "warn",
    ...
    const exchangeForToken = (tid: string, token: string, scopes: string[]): Promise<string> => {
        return new Promise((resolve, reject) => {
            ...
            const params = {
                client_id: process.env.PRONOUNCENAME_APP_ID,
                client_secret: process.env.PRONOUNCENAME_APP_SECRET,
